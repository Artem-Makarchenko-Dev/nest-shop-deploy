...
async function main() {
  ...
  if (!admin || !manager || !user || !adminRole || !managerRole || !userRole) {
    throw new Error('Failed to resolve seeded users or roles for UserRole linking');
  }
  ...
  const categoriesData = [
    ...
    { slug: 'mens-clothing', name: "Men's Clothing", description: 'Clothes for men' },
    { slug: 'womens-clothing', name: "Women’s Clothing", description: 'Clothes for women' },
  ];
  ...
  for (const product of productsData) {
    const existing = await prisma.product.findFirst({ where: { name: product.title } });
    if (!existing) {
      ...
      const categorySlug = product.category.replaceAll(/\s+/g, '-').toLowerCase();
